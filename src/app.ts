...
if (!checkPlatform()) {
    logger.error(failed + `Platform check failed: ${process.platform} platform not supported`);
...
resolveRulesetbuild().then(rulesetbuildsList => {
    ...
    logger.log(ask + `Which ruleset do you want to install to OSU!Lazer(${osuPath})?(input answer below):`);
