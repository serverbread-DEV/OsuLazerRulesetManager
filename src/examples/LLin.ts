...
export default (async ()=> {
    ...
    class LLin extends Rulesetbuild {
        // Ruleset 的基本信息
        override name = "LLin";
        override description = " 从mfosu中分离出来的下载加速 & 游戏内音乐播放器";
        ...
        override build() {
            ...
            this.getResource(
                "git@github.com:MATRIX-feather/LLin.git",
                "git"
            );
            this.log("Building...");
            ...
            this.cmd("dotnet publish IGPlayerLoader -c Release /p:WarningLevel=0");
            this.log("Build Complete!");
            this.log("Packaging...")
            // Artifacts saved in IGPlayerLoader/bin/Release/net8.0/publish/
            // Rulesetbuild#package()：将构建产物打包。
            const artifactsDir = resolve(this.tempDir, this.name, "LLin", "IGPlayerLoader/bin/Release/net8.0/publish/");
            const fileList = [
                "M.*.dll", "*/M.*.dll", "osu.Game.Rulesets.*.dll", "Tmds.*.dll"
            ]
